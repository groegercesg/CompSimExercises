...
class Decay(object):
    ...
    def decay_simulation(self):
        ...
        p = self.decayCons * self.delta_t
        # probability that a nucleus decays within a given time period
        hl_sim = 0
        # half-life based on simulation, to be incremented by while loop
        hl_act = round((math.log(2) / self.decayCons), 2)
        Nuc_initial = self.N * self.N  # Initial number of undecayed nuclei

        # while loop until over half of nucleuses have been decayed
        ...
        self.print_nuclei()
        print("\n")
        print("Initial number of undecayed nuclei: " + str(Nuc_initial))
...
def main():
    ...
    try:
        decaycons = float(input(
            "What is the value of your decay constant: \n"))
