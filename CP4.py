...
class Traffic(object):
    ...
    def initialise(self):
        ...
        placed_cars = 0 # counter for number of placed cars so we don't exceed our desired CarDensity
            ...
            for i in range(0, self.RoadLength):
                if self.Road[j][i] == 1:
                    ...
                    if self.Road[j][(i-1)%self.RoadLength] == 1:
                        ...
                        self.Road[j+1][i] = 0
                        #car_moves += 1
            ## average_speeds = average_speeds + " - " + str(car_moves) + " . " + str(self.average_speed(car_moves))
