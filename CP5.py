...
class Simulation(object):
    ...
    def __init__(self, myFileA, myFileB):
        ...
        self.yvb = math.sqrt((Simulation.G * self.ma)/(self.xb)) # v_{2} = \sqrt{(G * m_{1})/(r_{12})}
    ...
    def TotalKE(self):
        ...
        # the sum of individual objects kinetic energies, with KE = 1/2 * m * v^{2}
        # and so v^{2} term calculated using pythagoras of x and y velocities, so square
        # root squared is just the underlying components
        return ((0.5 * self.ma * (self.xva**2 + self.yva**2)) + (0.5 * self.mb * (self.xvb**2 + self.yvb**2)))
    ...
    def runSim(self):
        ...
        # counter to track number of iterations, so as to print Total KE at suitable intervals (roughly at every 10% of iterations)
        count = 0
        ...
        while t < Simulation.DaySec: # orbital period is apparently 0.319 * DaySec, but we'll roll with this for now
            ...
            # incriment t to move simulation forward
            t += dt
            
            # Save positions in lists, these lists being for use in Animation classes methods
...
class Animation(object):
    ...
    def animate(self, i):
        # create 2 to be animated, using parameter i to move their position, each with individual arrays
    ...
    def display(self):
        ...
        # create circles centred at initial position and then append them to the list
        self.patches.append(plt.Circle((self.xapos[0], self.yapos[0]), 250000, color='orange', animated=True))
        self.patches.append(plt.Circle((self.xbpos[0], self.ybpos[0]), 250000, color='thistle', animated=True))
